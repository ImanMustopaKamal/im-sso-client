  ...
  <div class="card">
    <div class="card-header justify-content-between">
      <div class="card-header-action">
        ...
        @click="()=>{
          this.$emit('Modal', 'organization_modal', true)
        }">
        ...
        <template v-else>
          <tbody v-if="items.length > 0">
            <tr
              v-for="item in items"
              :key="item.id"
              @click="OnClickItemOrganizationList(item.id)"
            >
...
export default {
  ...
  methods: {
    ...
    OnClickItemOrganizationList(id){
      this.$emit('Modal', 'organization_modal', true, id)
    },
    DeleteOrganization(item) {
      this.$emit(
        'Confirm', 
        HelperMessage.getMessage("message.modal.dialog.confirm", {
          name: "delete this Organization",
        }),
        () => {
          this.$emit('ShowLoader', true)
          this.Delete({ url: `/tenants/${this.user.tenant_id}/organizations/${item}`})
          .then(() => {
