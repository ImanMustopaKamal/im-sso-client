  ...
  <div class="card">
    <div class="card-header justify-content-between">
      <div class="card-header-action">
        ...
        @click="()=>{
          this.$emit('Modal', 'unit_modal', true)
        }">
        ...
        <template v-else>
          <tbody v-if="items.length > 0">
            <tr
              v-for="item in items"
              :key="item.id"
              @click="OnClickItemUnitList(item.id)"
            >
...
export default {
  ...
  methods: {
    ...
    OnClickItemUnitList(id){
      this.$emit('Modal', 'unit_modal', true, id)
    },
    DeleteUnit(item) {
      this.$emit(
        'Confirm', 
        HelperMessage.getMessage("message.modal.dialog.confirm", {
          name: "delete this Business Unit",
        }),
        () => {
          this.$emit('ShowLoader', true)
          this.Delete({ url: `/tenants/${this.user.tenant_id}/business-units/${item}`})
          .then(() => {
