  ...
  <div class="card">
    <div class="card-header justify-content-between">
      <div class="card-header-action">
        ...
        @click="()=>{
          this.$emit('Modal', 'subunit_template_modal', true)
        }">
        Add New Business Subunit Template
        </button>
      </div>
      <div class="d-flex">
        <button
      ...
      >
        <thead>
          <tr>
            ...
            <th class="text-center">Business Unit Template</th>
            <th class="text-center">Action</th>
          </tr>
        </thead>
        <template v-if="loading_data">
          <Shimmer :row="5" :column="4" />
        </template>
        <template v-else>
          <tbody v-if="items.length > 0">
            <tr
              v-for="item in items"
              :key="item.id"
              @click="OnClickItemSubunitTemplateList(item.id)"
            >
              <td class="text-center">{{ item.id }}</td>
              <td>{{ item.name ? item.name : "N/A" }}</td>
              <td>{{ item.business_unit_template_name ? item.business_unit_template_name : "N/A" }}</td>
...
export default {
  ...
  methods: {
    ...
    OnClickItemSubunitTemplateList(id){
      this.$emit('Modal', 'subunit_template_modal', true, id)
    },
    DeleteSubunitTemplate(item) {
      this.$emit(
        'Confirm', 
        HelperMessage.getMessage("message.modal.dialog.confirm", {
          name: "delete this Business Subunit Template",
        }),
        () => {
          this.$emit('ShowLoader', true)
          this.Delete({ url: `/tenants/${this.user.tenant_id}/subunit-templates/${item}`})
          .then(() => {
