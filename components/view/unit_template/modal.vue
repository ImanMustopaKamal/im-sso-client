<template>
  <div>
    ...
    >
      <div class="row">
        <div class="col-12">
          <form @submit.prevent="SubmitUnitTemplate" method="POST">
            <div class="form-row">
              <div class="col">
...
export default {
  ...
  methods: {
    ...
    ShowModal(state, id = null) {
      ...
      if (state) {
        if (id) {
          this.title = "Edit Unit Template";
          this.GetData(id).then(() => {
            this.show_modal = true;
          });
        } else {
          this.title = "Add New Unit Template";
          this.form = {
            name: "",
            description: "",
          };
    ...
    async GetData(id) {
      try {
        ...
        let get_data = await this.$store.dispatch("library/GetData", {
          url: `/tenants/${this.user.tenant_id}/business-unit-templates/${id}`,
        });
    ...
    SubmitUnitTemplate() {
      ...
      if (Object.keys(do_validation).length > 0) {
        ...
        this.$emit(
          ...
          () => {
            this.show_modal = false;
            this.$emit("SetFilter", "unit_template_view", {});
          }
        );
      } else {
        let message = this.id
          ? "update this Unit Template"
          : "submit this Unit Template";
        this.$emit(
          "Confirm",
    ...
    PostData() {
      ...
      if(this.id){
        this.Update({ url: `/tenants/${this.user.tenant_id}/business-unit-templates/${this.id}`, request_body: data })
        .then(() => {
          this.$emit("ShowLoader", false);
          this.$emit(
            "Success",
        ...
        this.Insert({ url: `/tenants/${this.user.tenant_id}/business-unit-templates`, request_body: data })
        .then(() => {
          this.$emit("ShowLoader", false);
          this.$emit(
            "Success",
