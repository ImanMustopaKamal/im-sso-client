  ...
  <div class="card">
    <div class="card-header justify-content-between">
      <div class="card-header-action">
        ...
        @click="()=>{
          this.$emit('Modal', 'unit_template_modal', true)
        }">
        Add New Unit Template
        </button>
      </div>
      <div class="d-flex">
        <button
        ...
        <template v-else>
          <tbody v-if="items.length > 0">
            <tr
              v-for="item in items"
              :key="item.id"
              @click="OnClickItemUnitTemplateList(item.id)"
            >
...
export default {
  ...
  methods: {
    ...
    OnClickItemUnitTemplateList(id){
      this.$emit('Modal', 'unit_template_modal', true, id)
    },
    DeleteUnitTemplate(item) {
      this.$emit(
        'Confirm', 
        HelperMessage.getMessage("message.modal.dialog.confirm", {
          name: "delete this Unit Template",
        }),
        () => {
          this.$emit('ShowLoader', true)
          this.Delete({ url: `/tenants/${this.user.tenant_id}/business-unit-templates/${item}`})
          .then(() => {
