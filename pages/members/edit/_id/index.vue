...
export default {
  ...
  methods: {
    ...
    async GetData(){
      try{
        ...
        let get_data = await this.$store.dispatch(
          'library/GetData',
          { url: `/tenants/${this.user.tenant_id}/users/${id}` }
        )

        if(get_data.data.length > 0){
          let phone = get_data.data[0].phone
          if(phone && phone == '+62838'){
            phone = ""
          }else if (phone && phone != '+62838') {
          ...
          this.form = {
            ...
            employee_number: get_data.data[0].employee_number,
            phone_number: phone,
            email: get_data.data[0].email,
            organization: get_data.data[0].organization_id,
            business_unit: get_data.data[0].business_unit_id,
            subunit: get_data.data[0].subunit_id,
            status: get_data.data[0].user_status_id
          }
          this.old_data = {
            name: get_data.data[0].name,
            employee_number: get_data.data[0].employee_number,
        ...
        this.GetDDLBusinessUnit(this.form.business_unit)
        this.GetDDLSubunit(this.form.subunit)
        
        this.loader = false
      }catch(err){
        this.loader = false
        this.ShowModalError(err.message, false, this.RedirectHome)
      }
    },
    async GetDDLStatus() {
      try{
    ...
    async GetDDLSubunit(subunit_selected = null) {
      try{
    ...
    IsDataNotEdit() {
      return JSON.stringify(this.form) === JSON.stringify(this.old_data)
    },
    OnSubmit(){
      ...
      if (do_validation) {
        ...
        this.ShowModalConfirm(
          HelperMessage.getMessage("message.modal.dialog.edit", {
            name: "Member",
          }),
          this.PostData
        );
    ...
    PostData(){
      ...
      this.Update({ url: `/tenants/${this.user.tenant_id}/users/${id}`, request_body: data })
      .then(() => {
        this.loader = false
